...
(CSLeft_B<=60&&(CSLeft_G<=100&&CSLeft_G>=54)&&CSLeft_R<=60)))
            ...
            else if(US_Front<=15||US_Left<=12||US_Right<=12)
            {
                Duration = 1;
                CurAction =13;
            }
        ...
        private static void Game1()
        {
            ...
            else if(US_Front>=245 && US_Front<=255 && US_Back>=230 && US_Back<=255 && US_Left>=225 && US_Left<=255 && US_Right>=240 && US_Right<=255 && CSLeft_R>=80 && CSLeft_R<=255 && CSLeft_G>=255 && CSLeft_G<=255 && CSLeft_B>=145 && CSLeft_B<=255)
            {
                Duration = 0;
                CurAction =1;
            }
